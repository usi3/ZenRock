...
* ブラウザから録画した番組を閲覧・検索
	* 検索はスペース区切りでOR検索，単語の先頭に"-"を付けると除外検索になります（例：「選挙 -AKB」「群馬 -野菜」「料理 -グルメ -旅」）
	* 番組の予約を担当するプログラム（後述する tvbooker.exe）を使わなければ，ユーザが手動で録画した番組に対してメタ情報を付加し，ブラウザからその一覧を閲覧・検索することもできます

## 動作環境
* ネットワークに接続され地デジチューナを搭載しているPC
* OS: Windows 7 (64-bit)
* ストレージ容量の目安: 170×録画を録りためたい日数×録画したい放送局数[GB]
...
2. setting.jsonを設定
	...
	* パスに日本語や()が含まれていると問題が起きます．詳しくは次のissueをご覧ください https://github.com/usi3/ZenRock/issues/1
...
※Rubyなしで使い始められるようにソースコードに加えて実行ファイルを配布していましたが，これはあまり意味がありませんでした．現在はバイナリの実行にもプログラムの開発環境が必要な状況です．
...
### tvinfocollector.exe
このプログラムはセキュリティの都合上プロプライエタリとしています．
次の処理を行います．
...
## 動作実績
* Dell XPS 8500をベースにPLEX PX-W3PE×2, PLEX PX-Q3PE, 3TB SATA HDD×2（スパンボリュームとして運用）を追加したPC(Windows 7 64-bit)にて2012年6月～2013年8月まで運用
