# 異常操作について考えていないので、決してポート開放しないこと
require 'rubygems'
require 'cgi'
require 'webrick'
require 'kconv'
...
# 稼動テスト
# http://localhost:10080/hello?name=moomoofarm
...
class ClientServlet < WEBrick::HTTPServlet::AbstractServlet
	def do_GET(req, res)
		...
		if req.query['cmd'] == "getall"
			...
			# http://localhost:10080/exec?cmd=rate&serialid=ddddddddd&videoid=120120821122000&rate=3.0
			...
			# http://localhost:10080/exec?image=120120723150000
			# http://localhost:10080/exec?image=220120723153500
...
class UIServlet < WEBrick::HTTPServlet::AbstractServlet
	def convertAllToHTML(sid, search)
		...
		# newest order 
		all.sort!{|a, b| b["id"].to_s[4..-1] <=> a["id"].to_s[4..-1]}
	...
	def do_GET(req, res)
		...
		if req.query['watch'] != nil
			...
			cmd = "#{$vlcpath} file:///#{$mediaDir}#{videoid}.ts --quiet"
			puts cmd
